...
impl Params {
    ...
    fn serialize(&self) -> PyResult<PyObject> {
        ...
        let bytes: PyObject = Python::with_gil(|py| PyBytes::new(py, &data).into());
        Ok(bytes)
    }
}
...
fn xla_registrations() -> PyResult<PyObject> {
    ...
    let dict = Python::with_gil(|py| -> PyResult<PyObject> {
        ...
        let rust_fn: &PyCapsule = unsafe { py.from_owned_ptr_or_err(rust_fn)? };
        let dict = [("rust_fn", rust_fn)].into_py_dict(py);
        Ok(dict.into())
    })?;
    Ok(dict)
}
...
fn jflash_attn(_py: Python<'_>, m: &PyModule) -> PyResult<()> {
