...
def run_mha_bwd(is_causal, softmax_scale, softcap, res, grad):
    ...
    _b_sz, _seqlen_q, num_heads, _head_size = q.shape
    b_sz, seqlen_k, num_heads_k, head_size = k.shape
    grad_q, grad_k, grad_v, softmax_d, dq_accum = _run_mha_bwd.bind(
        grad,
        output,
        softmax_lse,
        q,
...
def _run_mha_fwd_cuda_lowering(ctx, q, k, v, is_causal, softmax_scale, softcap):
    ...
    window_size_right = 0 if is_causal else seqlen_k
    ...
    out = custom_call(
        ...
        operand_layouts=default_layouts(q_type.shape, k_type.shape, v_type.shape),
...
def _run_mha_bwd_cuda_lowering(
    ...
    out = custom_call(
        ...
        result_layouts=default_layouts(
            dq_shape, dk_shape, dv_shape, softmax_d_shape, dq_accum_shape
        ),
    )
...
def _run_mha_fwd_abstract(q, k, v, is_causal, softmax_scale, softcap):
